# 概要
π³ / Pi3X と SAM を組み合わせ、RGB動画のインプットからオブジェクトを出力するリポジトリです。詳細はREADME.mdを確認してください。

# ルール

## 共通
- デフォルトでエージェントチームズ機能を用い、複数の視点や枠割を使い分けて挑むようにしてください。
- タスク機能を用い、マクロなタスク（phase-{n}）とミクロなタスク（phase-{n}_{m}）という形で計画を建てるようにしてください。

## 設計フェーズの運用ルール（Spec→MVP thin slice）

### 進め方
- まず AskUserQuestion で“ブレると手戻りが大きい判断”だけを集中的に聞く（UI/UX、データ、権限、失敗時、トレードオフ）。
- 回答が揃ったらタスク管理機能でタスクを作成し、設計を固定してから実装へ（実装は新セッション推奨）。
...
### デフォルトの進め方
- ローカルでの実行実現性を考慮する。
  - もし追加・編集したコードを検証しようとしても、現状のマシンでは動作しないのでは検証できない。この際にはスキップしても問題ない。
  - ただし、リントチェックや、入出力関係の大まかな検証について、できる範囲のことは行うこと
