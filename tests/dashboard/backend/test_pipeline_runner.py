...
class TestBuildStageProgressPayload(unittest.TestCase):
    ...
    def test_session_updated(self) -> None:
        session = PipelineSession()
        session.stage_start(PipelineStage.GWRAPPING_RECONSTRUCT)

        _build_stage_progress_payload(
            session,
            PipelineStage.GWRAPPING_RECONSTRUCT,
            progress=75.0,
            detail="Running gwrapping",
        )

        info = session.stages[int(PipelineStage.GWRAPPING_RECONSTRUCT)]
        self.assertEqual(info.progress, 75.0)
        self.assertEqual(info.detail, "Running gwrapping")
