...
_stub_gwrapping = types.ModuleType("scripts.stage_gwrapping_reconstruct")
...
_STUBS = {
    ...
    "scripts.stage_gwrapping_reconstruct": _stub_gwrapping,
...
class _WrapperTestBase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.mock_gwrapping = MagicMock(name="run_gwrapping")
        _stub_gwrapping.run_gwrapping = self.mock_gwrapping  # type: ignore[attr-defined]
...
class TestStageGWrappingVRAMCleanup(_WrapperTestBase):
...
class TestLazyImportVerification(_WrapperTestBase):
    ...
    def test_gwrapping_calls_underlying(self) -> None:
        _stage_gwrapping_reconstruct(
            "/frames", "/colmap_sparse", "/masks", "/out",
        )
        self.assertTrue(self.mock_gwrapping.called)
