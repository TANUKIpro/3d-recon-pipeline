  ...
  describe('getConfig', () => {
    ...
    it('all expected keys present', () => {
      ...
      const expectedKeys = [
        ...
        'gwrapping_preset', 'gwrapping_iterations', 'gwrapping_rasterizer',
        'gwrapping_resolution', 'gwrapping_use_masks',
        'gwrapping_use_depth', 'gwrapping_extraction_method',
    ...
    it('empty inputs produce default values', () => {
      ...
      expect(config.gwrapping_preset).toBe('default');
    ...
    it('reads the selected gwrapping rasterizer', () => {
      document.getElementById('cfg-gwrapping-rasterizer').value = 'radegs';
      const config = panel.getConfig();
      expect(config.gwrapping_rasterizer).toBe('radegs');
    });

    it('reads the selected gwrapping preset', () => {
      document.getElementById('cfg-gwrapping-preset').value = 'high';
      const config = panel.getConfig();
      expect(config.gwrapping_preset).toBe('high');
    });
  });

  describe('gwrapping presets', () => {
    it('applies High preset values to public Stage 4 inputs', () => {
      const presetSelect = document.getElementById('cfg-gwrapping-preset');
      presetSelect.value = 'high';
      presetSelect.dispatchEvent(new Event('change', { bubbles: true }));

      expect(document.getElementById('cfg-gwrapping-iterations').value).toBe('30000');
    });
    ...
    it('marks the preset as custom after a Stage 4 public input edit', () => {
      ...
      const iterations = document.getElementById('cfg-gwrapping-iterations');
      iterations.value = '50000';
      iterations.dispatchEvent(new Event('input', { bubbles: true }));

      expect(presetSelect.value).toBe('custom');
      expect(panel.getConfig().gwrapping_preset_base).toBe('high');
    });
  });
