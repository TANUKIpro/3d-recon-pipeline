...
from scripts.config_defaults import (
    ...
    GWRAPPING_RASTERIZERS,
    GWRAPPING_RESOLUTION,
    GWRAPPING_USE_MASKS,
    GWRAPPING_USE_DEPTH,
    GWRAPPING_EXTRACTION_METHOD,
    GWRAPPING_EXTRACTION_METHODS,
...
from scripts.gwrapping_config import (
    GWRAPPING_CONFIG_FIELDS,
...
_GWRAPPING_ENV_FIELD_MAP: dict[str, str] = {
    "gwrapping_iterations": "GWRAPPING_ITERATIONS",
    "gwrapping_rasterizer": "GWRAPPING_RASTERIZER",
    "gwrapping_resolution": "GWRAPPING_RESOLUTION",
    "gwrapping_use_masks": "GWRAPPING_USE_MASKS",
}
_GWRAPPING_PRESET_CHOICES = set(GWRAPPING_PRESETS) | {"custom"}


def _parse_gwrapping_field_value(
    field_name: str,
    value: Any,
    fallback: Any,
) -> Any:
    if field_name == "gwrapping_iterations":
        return max(1000, parse_int(value, int(fallback)))
    if field_name == "gwrapping_rasterizer":
        return parse_choice(value, GWRAPPING_RASTERIZERS, str(fallback))
    if field_name == "gwrapping_resolution":
        return max(1, parse_int(value, int(fallback)))
    if field_name == "gwrapping_use_masks":
        return parse_bool(value, bool(fallback))
    if field_name == "gwrapping_use_depth":
        return parse_bool(value, bool(fallback))
    if field_name == "gwrapping_extraction_method":
        return parse_choice(value, GWRAPPING_EXTRACTION_METHODS, str(fallback))
    raise KeyError(f"Unknown gwrapping field: {field_name}")


def _apply_gwrapping_overrides(
    values: dict[str, Any],
    ...
    for field_name in field_names:
        if field_name not in source:
            continue
        values[field_name] = _parse_gwrapping_field_value(
            field_name,
...
def _apply_gwrapping_env_fallbacks(
    values: dict[str, Any],
    env_map: Mapping[str, str],
) -> dict[str, Any]:
    for field_name, env_name in _GWRAPPING_ENV_FIELD_MAP.items():
...
def _resolve_gwrapping_config(
    raw: Mapping[str, Any],
    env_map: Mapping[str, str],
    *,
    explicit_keys: set[str] | None,
) -> dict[str, Any]:
    raw_preset = normalize_preset(raw.get("gwrapping_preset"), fallback="")
    raw_preset_base = normalize_preset(raw.get("gwrapping_preset_base"), fallback="")
    env_preset = normalize_preset(env_map.get("GWRAPPING_PRESET"), fallback="")
    preserve_all_raw = explicit_keys is None
    stage4_keys_present = {key for key in GWRAPPING_CONFIG_FIELDS if key in raw}

    if preserve_all_raw:
        selected_preset = raw_preset or env_preset or GWRAPPING_PRESET
        base_preset = raw_preset_base if raw_preset_base in GWRAPPING_PRESETS else ""
        if raw_preset in GWRAPPING_PRESETS:
            base_preset = raw_preset
        elif raw_preset == "custom" and base_preset not in GWRAPPING_PRESETS:
            inferred = infer_preset_from_public_fields(raw)
            base_preset = inferred if inferred in GWRAPPING_PRESETS else GWRAPPING_PRESET
        elif selected_preset not in GWRAPPING_PRESETS:
            inferred = infer_preset_from_public_fields(raw)
            base_preset = inferred if inferred in GWRAPPING_PRESETS else GWRAPPING_PRESET
        else:
            base_preset = selected_preset
        values = config_fields_from_preset(base_preset)
        values = _apply_gwrapping_overrides(values, raw, GWRAPPING_CONFIG_FIELDS)
        if not stage4_keys_present and not raw_preset and not env_preset:
            values = _apply_gwrapping_env_fallbacks(values, env_map)
        if raw_preset in GWRAPPING_PRESETS:
            final_preset = (
                raw_preset
            ...
            final_preset = "custom"
        elif env_preset in GWRAPPING_PRESETS and not stage4_keys_present:
            final_preset = (
                env_preset
        ...
        values["gwrapping_preset"] = final_preset
        values["gwrapping_preset_base"] = (
            final_preset if final_preset in GWRAPPING_PRESETS else base_preset
        )
        return values

    explicit = set(explicit_keys)
    selected_preset = raw_preset or env_preset
    if selected_preset not in _GWRAPPING_PRESET_CHOICES:
        inferred = infer_preset_from_public_fields(raw)
        selected_preset = inferred if inferred in _GWRAPPING_PRESET_CHOICES else GWRAPPING_PRESET
    selected_base = raw_preset_base if raw_preset_base in GWRAPPING_PRESETS else ""
    if not selected_base:
        selected_base = selected_preset if selected_preset in GWRAPPING_PRESETS else GWRAPPING_PRESET

    if "gwrapping_preset" in explicit and selected_preset in GWRAPPING_PRESETS:
        values = config_fields_from_preset(selected_preset)
    else:
        if selected_preset == "custom":
            base_preset = selected_base
        else:
            base_preset = selected_preset if selected_preset in GWRAPPING_PRESETS else GWRAPPING_PRESET
        values = config_fields_from_preset(base_preset)
        values = _apply_gwrapping_overrides(values, raw, GWRAPPING_CONFIG_FIELDS)

    explicit_stage4_fields = {
        key for key in explicit if key in GWRAPPING_CONFIG_FIELDS
    }
    values = _apply_gwrapping_overrides(values, raw, explicit_stage4_fields)
    ...
    if "gwrapping_preset" in explicit and selected_preset in GWRAPPING_PRESETS:
        ...
        final_preset = "custom"
    elif selected_preset in GWRAPPING_PRESETS and fields_match_preset(values, selected_preset):
    ...
    values["gwrapping_preset"] = final_preset
    values["gwrapping_preset_base"] = (
        final_preset if final_preset in GWRAPPING_PRESETS else selected_base
    )
    return values
...
def build_pipeline_config(
    ...
    *,
    ...
    gwrapping_cfg = _resolve_gwrapping_config(
        raw,
        env_map,
    ...
    return PipelineConfig(
        ...
        gwrapping_preset=str(gwrapping_cfg["gwrapping_preset"]),
        gwrapping_preset_base=str(gwrapping_cfg["gwrapping_preset_base"]),
        gwrapping_iterations=int(gwrapping_cfg["gwrapping_iterations"]),
        gwrapping_rasterizer=str(gwrapping_cfg["gwrapping_rasterizer"]),
        gwrapping_resolution=int(gwrapping_cfg["gwrapping_resolution"]),
        gwrapping_use_masks=bool(gwrapping_cfg["gwrapping_use_masks"]),
        gwrapping_use_depth=bool(gwrapping_cfg["gwrapping_use_depth"]),
        gwrapping_extraction_method=str(gwrapping_cfg["gwrapping_extraction_method"]),
