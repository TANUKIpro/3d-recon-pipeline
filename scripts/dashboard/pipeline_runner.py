    ...
    try:
        ...
        if start_stage <= int(PipelineStage.SAM2_SEGMENT):
            ...
            await _wait_for_next_stage_confirmation(
                ...
                "SAM2 Segmentation complete. Continue to GaussianWrapping Reconstruction?",
            )

        if start_stage <= int(PipelineStage.GWRAPPING_RECONSTRUCT):
            _require_dir(session.frames_dir, "Extracted frames directory", must_have_suffix=".jpg")
            _require_file(session.poses_path, "Camera poses file")

            # VRAM gate: ensure sufficient free VRAM before GaussianWrapping
            with stage_log_scope(int(PipelineStage.GWRAPPING_RECONSTRUCT)):
            ...
            await _wait_for_next_stage_confirmation(
                ...
                "GaussianWrapping Reconstruction complete. Continue to Texture Bake?",
            )
